...
class OrderController extends Controller
{
  public function inspect(Request $request)
  {
    ...
    $users  = User::all();
    $status = Status::all();
    return view('inspect_order', ['order' => $order, 'users' =>$users, 'status' => $status]);
  }
  ...
  public function navCreate()
  {
    $devices = Device::all();
    $status  = Status::all();
    return view('create_order', ['devices' => $devices, 'status' => $status]);
  }
  ...
  public function create(Request $request) {
    ...
    if (Order::whereTitle($params['title'])->exists()) {
        return view('create_order', ['devices' => $devices, 'status' => $status, 'error' => 6]);
    }
    ...
    try {
      ...
      return view('create_order', ['devices' => $devices, 'status' => $status, 'error' => 3]);
    }
    ...
    try {
      ...
      $order->save();
        ...
        return view('create_order', ['devices' => $devices, 'status' => $status, 'error' => 1]);
    }
    ...
    try {
      foreach ($params as $key => $value) {
        ...
        dd($exception, $params);
        return view('create_order', ['devices' => $devices, 'status' => $status, 'error' => 2]);
    }
